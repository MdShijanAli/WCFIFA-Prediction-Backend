...
export const submitPrediction = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const submitBulkPredictions = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as any).userId;
    ...
    for (const pred of predictions) {
      try {
        ...
        if (!match || match.status !== 'SCHEDULED' || new Date() >= match.scheduledAt) {
          errors.push({ matchId: pred.matchId, error: 'Match not available for prediction' });
          continue;
        }

        if (pred.predictedWinnerId !== match.homeTeamId && pred.predictedWinnerId !== match.awayTeamId) {
        ...
        const result = await prisma.prediction.upsert({
          ...
          create: { userId, matchId: pred.matchId, predictedWinnerId: pred.predictedWinnerId },
    ...
    res.json({ saved: results.length, errors, message: `${results.length} predictions saved` });
  } catch (error) {
