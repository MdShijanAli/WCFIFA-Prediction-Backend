...
export const authApi = {
  ...
  verifyPhone: (userId: string, code: string) => api.post('/auth/verify-phone', { userId, code }),
  resendOTP: (userId: string, type: string) => api.post('/auth/resend-otp', { userId, type }),
...
export const paymentApi = {
  ...
  confirmDemo: (transactionId: string) => api.post('/payments/confirm-demo', { transactionId }),
...
export const predictionApi = {
  ...
  submitBulk: (predictions: Array<{ matchId: string; predictedWinnerId: string }>) =>
    api.post('/predictions/bulk', { predictions }),
  getMyPredictions: (round?: string) => api.get('/predictions', { params: { round } }),
};
