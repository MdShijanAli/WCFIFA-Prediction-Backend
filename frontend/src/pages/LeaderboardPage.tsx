...
export default function LeaderboardPage() {
  ...
  const getRankIcon = (rank: number) => {
    ...
    return (
      <span className="text-gray-500 text-sm font-bold w-5 text-center">
        #{rank}
      </span>
    );
  };
        ...
        {loading ? (
          <div className="p-6 space-y-3">
            {[1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className="bg-[#9494a8] h-16 rounded-xl animate-pulse"
              />
            ))}
          </div>
        ) : (
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-4 px-6 py-4 transition-colors ${
                    isMe ? "bg-[#F5C518]" : "hover:bg-[#F5C518]/10"
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isMe && (
                        <span className="text-xs bg-primary-700 text-primary-200 px-2 py-0.5 rounded-full">
                          You
                        </span>
                      )}
                    </div>
